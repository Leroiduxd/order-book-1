...
async function readDbRow(id) {
  return withDb(async () => {
    ...
    const row = (await pgGet(`positions?id=eq.${id}&select=${sel}`))?.[0] || null;
    return row ? {
      id: Number(row.id),
...
async function patchDbState(id, state) {
  return withDb(async () => pgPatch(`positions?id=eq.${id}`, { state }));
}

// ---------- Core ----------
async function reconcileStateOnly(id) {
  ...
  if (db.state === 0 && chainState === 1) {
    ...
    try {
      if (entryX6 !== 0n) {
        await withDb(() => handleExecutedEvent({ id, entryX6: entryX6 }));
        out.executed++;
      } else {
        // Si aucun prix en DB, on bascule l’état au minimum
        await patchDbState(id, 1);
        out.patched++;
      }

      // Indexer SL/TP antagonistes si non nuls (liq est souvent gérée ailleurs)
  ...
  if (db.state === 1 && (chainState === 2 || chainState === 3)) {
    try {
      ...
      if (chainState === 3) {
        await patchDbState(id, 3); // passer explicitement à CANCELLED si besoin
      }
      out.removed++;
  ...
  try {
    await patchDbState(id, chainState);
...
(async () => {
  ...
  async function workerMain(wid) {
    while (true) {
      ...
      const id = ids[i];
      const r = await reconcileStateOnly(id);
      acc.scanned++;
